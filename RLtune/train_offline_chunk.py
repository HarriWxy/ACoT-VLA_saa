...
# ---------------------------------------------------------------------------
# Tree splitting utility for gradient accumulation
# ---------------------------------------------------------------------------

from typing import Any

def _split_tree(tree: Any, n_chunks: int) -> list[Any]:
    """Split a nested dict / Observation / tensor tree along batch dim (dim=0).

    Returns a list of ``n_chunks`` sub-trees, each containing a slice of the
    original tensors.  Works recursively on dicts, lists, tuples, dataclasses,
    and objects with ``__dataclass_fields__`` (e.g. ``Observation``).
    """
    if isinstance(tree, torch.Tensor):
        return list(torch.chunk(tree, n_chunks, dim=0))
    if isinstance(tree, dict):
        per_chunk: list[dict] = [{} for _ in range(n_chunks)]
        for k, v in tree.items():
            chunks = _split_tree(v, n_chunks)
            for i, c in enumerate(chunks):
                per_chunk[i][k] = c
        return per_chunk
    if isinstance(tree, (list, tuple)):
        per_chunk_list: list[list] = [[] for _ in range(n_chunks)]
        for item in tree:
            chunks = _split_tree(item, n_chunks)
            for i, c in enumerate(chunks):
                per_chunk_list[i].append(c)
        return [type(tree)(x) if isinstance(tree, tuple) else x for x in per_chunk_list]
    if hasattr(tree, "__dataclass_fields__"):
        per_chunk_dc: list[dict] = [{} for _ in range(n_chunks)]
        for field_name in tree.__dataclass_fields__:
            chunks = _split_tree(getattr(tree, field_name), n_chunks)
            for i, c in enumerate(chunks):
                per_chunk_dc[i][field_name] = c
        return [type(tree)(**d) for d in per_chunk_dc]
    # Objects with __dict__ (e.g. SimpleNamespace) — split their attributes
    if hasattr(tree, "__dict__"):
        per_chunk_obj: list[dict] = [{} for _ in range(n_chunks)]
        for attr_name, attr_val in vars(tree).items():
            chunks = _split_tree(attr_val, n_chunks)
            for i, c in enumerate(chunks):
                per_chunk_obj[i][attr_name] = c
        result = []
        for d in per_chunk_obj:
            obj = object.__new__(type(tree))
            obj.__dict__.update(d)
            result.append(obj)
        return result
    # Scalars / strings / None — just replicate
    return [tree] * n_chunks
...
class OfflineGRPOConfig:
    ...
    gradient_accumulation_steps: int = 4  # Split batch into micro-batches to reduce peak GPU memory
...
def train_loop(rl_config: OfflineGRPOConfig):
    ...
    for epoch in range(start_epoch, rl_config.total_epochs):
        ...
        for step in pbar:
            ...
            adv_np = adv_per_frame[:batch_size]

            # ── Gradient accumulation: split batch into micro-batches ──
            accum_steps = max(1, rl_config.gradient_accumulation_steps)
            # Clamp accum_steps to batch_size (can't split into more chunks than samples)
            accum_steps = min(accum_steps, batch_size)

            if accum_steps > 1:
                obs_chunks = _split_tree(obs_batch, accum_steps)
                action_chunks = np.array_split(actions_np, accum_steps, axis=0)
                adv_chunks = np.array_split(adv_np, accum_steps, axis=0)
            else:
                obs_chunks = [obs_batch]
                action_chunks = [actions_np]
                adv_chunks = [adv_np]

            step_metrics_agg: dict[str, float] = {}
            last_loss_val = 0.0

            for micro_idx in range(accum_steps):
                # Move micro-batch to device
                observation = jax_tree_to_device(obs_chunks[micro_idx], device)
                micro_actions = torch.from_numpy(action_chunks[micro_idx]).float().to(device)
                micro_adv = torch.from_numpy(adv_chunks[micro_idx]).float().to(device)

                # Project actions to model space if needed
                model_actions = micro_actions
                if projection_head is not None:
                    model_actions = projection_head.project_to_model(micro_actions)

                # Forward + loss
                loss, micro_metrics = compute_rl_loss(
                    model,
                    observation,
                    model_actions,
                    micro_adv,
                ...
                # Aggregate metrics (Python floats, safe to accumulate)
                last_loss_val = loss.item()
                for k, v in micro_metrics.items():
                    if isinstance(v, (int, float)):
                        step_metrics_agg[k] = step_metrics_agg.get(k, 0.0) + v / accum_steps

                # Free micro-batch tensors eagerly
                del observation, micro_actions, micro_adv, loss, scaled_loss

            # ── Gradient step (once per effective batch) ──
            ...
            # Release gradient memory fragmentation
            if accum_steps > 1:
                torch.cuda.empty_cache()

            global_step += 1
            step_losses.append(step_metrics_agg)
            ...
            if is_main and step % rl_config.log_interval == 0 and step > 0:
                writer.add_scalar("step/rl_loss", step_metrics_agg.get("rl_loss", last_loss_val), global_step)
                ...
                writer.add_scalar("step/grad_accum_steps", accum_steps, global_step)
                for key, value in step_metrics_agg.items():
