...
def train_loop(config: _config.TrainConfig):
    ...
    # --- LoRA: freeze non-LoRA parameters ---
    base_model = model.module if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model
    is_lora = getattr(base_model, "_lora_injected", False)
    if is_lora:
        base_model.freeze_non_lora_params()
        trainable, total = base_model.count_trainable_params()
        logging.info(f"LoRA mode: {trainable / 1e6:.2f}M trainable / {total / 1e6:.2f}M total")
    ...
    # Only train trainable parameters when LoRA is active
    if is_lora:
        trainable_params = [p for p in model.parameters() if p.requires_grad]
        logging.info(f"Optimizer: {len(trainable_params)} trainable parameter groups (LoRA mode)")
    else:
